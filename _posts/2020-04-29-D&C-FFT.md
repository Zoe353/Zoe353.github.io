...
<bf>Selection</bf>: pick some points $x_0, x_1, ...,x_{n-1}$, where n $\geq 2d+1$.  -- just linear time  
<bf>Evaluation</bf>: compute $A(x_0), A(x_1), ..., A(x_{n-1})$ and $B(x_0), B(x_1), ..., B(x_{n-1})$  
<bf>Multiplication</bf>: compute $C(x_k) = A(x_k)B(x_k)$ for $k=0,..,n-1$  --n multiplications, just linear time  
<bf>Interpolation<bf>: Recover $C(x) = c_0 + c_1 x + ...+c_{2d} x^{2d}$  
